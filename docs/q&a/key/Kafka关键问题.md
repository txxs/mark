1. Kafka一致性（可靠性）原理。生产者：生产的消息需要被ac，有三种ack的方式，分别是全部副本要ack；全部副本不需要ack；还有就是ISR，ISR是在延迟时间内可以和leader保持一致的副本。
   另外还有幂等：通过produceID+序列号的方式实现。而消费者这边也是两种，分别是高水位，只有高水位的消息才会被消费，这样就保证了副本的一致性；另外就是消费了的消息才会被提交，保证了单副本的一致性。 
   还有这个：
