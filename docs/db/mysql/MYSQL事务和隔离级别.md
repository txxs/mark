...
T1 读取某个**范围**的数据，T2 在这个范围内插入新的数据，T1 再次读取这个范围的数据，此时读取的结果和和第一次读取的结果不同。**这就是间隙锁可以解决的原因**
...
#### Read uncommitted（脏读，读已提交解决这个问题）
...
#### Read committed（不可重复读，可重复读解决这个问题，解决了select）
...
#### Repeatable read（存在幻读问题，序列化读解决，解决了update）
...
#### Serializable 序列化(解决了幻读，解决了insert)
